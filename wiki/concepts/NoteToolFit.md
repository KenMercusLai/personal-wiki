...
## Current Synthesis
The sources' shared conclusion is that note-taking tools are not neutral containers, but they are also not ends in themselves. Small-note systems need features that make many small files usable, such as backlinks, link suggestions, graph analysis, metadata, templates, quick capture, and fast switching. Big-note systems need features that make long documents manageable, such as outlines, heading navigation, folding, table-of-contents generation, visual anchors, block links, and text transport. The reading-note source adds that tool fit is also domain-specific: local graphs, backlinks, graph grouping, task plugins, and spoiler-folding display controls become useful because books, authors, awards, genres, and plot summaries create particular navigation and disclosure problems. Liang Mouyin's essay adds a personal-friction test: tool choice works when it simplifies the user's real workflow, not when it extends comparison, imitation, and setup.
...
- Domain-specific workflows may need specialized affordances such as graph filtering, spoiler controls, or list/task views.
...
The sources warn against chasing fashionable plugins or graph views for their own sake. Tool fit matters because tools support or frustrate a method and domain, not because every new feature is worth adopting; the reading-note example is valuable because the features solve concrete reading problems rather than merely decorating a note graph. Liang Mouyin's simplified stack is likewise personal rather than universal evidence; a larger toolchain can still fit someone with heavier research, collaboration, or archival needs.

## What Changed
- Created the concept to capture the source's claim that note methods and note software should be evaluated together.
- Added domain fit: reading notes need affordances for links, graphs, booklists, disclosure, and navigation.
- Added personal simplification as a tool-fit criterion when tool comparison becomes an attention cost.
