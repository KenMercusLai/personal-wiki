...
## Current Synthesis
RORIRI uses the formula "observation = model + error" to make statistical literacy more than memorized formulas. Learners should understand that every abstraction is lossy, that model choices embed value judgments, and that real data require scrutiny of sampling, bias, systematic error, and specification search. The later variance article makes this concrete: before interpreting a formula, learners should ask what [[DataGeneratingProcess]] produced the data, whether variables are [[Orthogonality|orthogonal]] in the sample, and how [[Covariance]] terms affect the total system. This makes statistics education a practical epistemic discipline for working with AI and complex reality.
...
- Independence in the generating process should not be confused with perfect zero correlation in observed noisy samples.
...
The sources criticize formula-heavy statistics education but do not reject formulas. They treat formulas as useful starting expressions that should lead to interpretable mental models, simulations, and checks against the data-generating story. The education proposal for R-based statistical experiments remains an educational design suggestion rather than evaluated outcome data, and the variance article's code examples are pedagogical simulations rather than a complete treatment of causal identification.

## What Changed
- Added variance, covariance, DGP, and orthogonality as concrete mechanics for the earlier education-oriented model-thinking frame.
