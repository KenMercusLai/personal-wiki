...
## Overview
[[TwilioSegment]] is the customer-data infrastructure product context used in the wiki for event fan-out architecture and, through Segment Sources, startup data integration for funnel analysis.

## Current Profile
Twilio Segment appears in the wiki as customer-data infrastructure at two levels. At the pipeline level, it receives customer events, decides which destinations should receive them, transforms payloads for each partner API, and handles retryable failures. Its architecture shifted as scale changed: destination isolation was important when one partner outage could block all delivery, but more than 140 services and queues eventually overloaded the team's operational capacity.

At the analytics-stack level, Segment Sources helps connect fragmented data so companies can analyze customer funnels across marketing, sales, customer success, lifecycle marketing, and upsell stages when paired with tools such as [[Looker]] Blocks.
...
- Appears in a separate source as a data-integration product that helps unify fragmented SaaS business data.
...
- Funnel data integration: [[all-new-ideas-are-combinations-of-old-ideas]] cites Segment Sources plus [[Looker]] Blocks as enabling analysis across marketing campaign, landing page, contract signing, customer success, lifecycle marketing, and upsell.

## Qualifications
The architecture source is a 2018 engineering retrospective focused on server-side destinations. The Tunguz source is a 2016 strategy essay citing Segment Sources as a data-stack example. Together they do not describe all Twilio Segment products, later architecture changes, current capabilities, customer metrics, or independent performance measurements.

## What Changed
- Created Twilio Segment as the company/product context for the destination pipeline architecture migration.
- Added Segment Sources as a data-integration example for cross-functional funnel analysis.
